...
# Default conflict resolution rules — Platinum Tier zone-scoped
DEFAULT_RULES = {
    # Cloud-wins: cloud agent is authoritative writer for these paths
    "Drafts/**": "cloud_wins",
    "Signals/**": "cloud_wins",
    "Needs_Action/cloud/**": "cloud_wins",
    "Updates/**": "remote_wins",
    # Local-wins: local agent is authoritative writer for these paths
    "Approved/**": "local_wins",
    "Done/**": "local_wins",
...
{section}"""
    ...
    def _log_sync(self, event: Dict[str, Any]) -> None:
        """Append to Logs/vault_sync.json and write individual SyncEvent file."""
        log_file = self.vault / "Logs" / "vault_sync.json"
        log = self._read_log()
        log.append(event)
        log_file.write_text(json.dumps(log, indent=2, default=str), encoding="utf-8")

        # Write individual SyncEvent per data-model contract (T024)
        sync_dir = self.vault / "Logs" / "sync"
        sync_dir.mkdir(parents=True, exist_ok=True)
        ts = event.get("timestamp", datetime.now(timezone.utc).isoformat())
        safe_ts = ts.replace(":", "-").replace("+", "p")[:19]
        sync_id = f"sync-{safe_ts}"
        sync_file = sync_dir / f"{sync_id}.json"
        sync_event = {
            "sync-id": sync_id,
            "event": event.get("event", "unknown"),
            "success": event.get("success", True),
            "files-changed": event.get("files_changed", []),
            "conflicts-resolved": event.get("conflicts_resolved", 0),
            "timestamp": ts,
        }
        sync_file.write_text(json.dumps(sync_event, indent=2, default=str), encoding="utf-8")
...
"""
        ...
        # Write conflict signal per sync_conflict_schema contract (T027)
        self._write_conflict_signal(files)

    def _write_conflict_signal(self, files: List[str]) -> None:
        """Write unresolvable conflicts to Signals/sync/ per sync_conflict_schema."""
        now = datetime.now(timezone.utc)
        signals_dir = self.vault / "Signals" / "sync"
        signals_dir.mkdir(parents=True, exist_ok=True)

        safe_ts = now.strftime("%Y%m%d-%H%M%S")
        conflict_id = f"conflict-{safe_ts}"
        ...
        signal_file = signals_dir / f"{conflict_id}.yaml"
        signal_file.write_text(
            yaml.dump(signal_data, default_flow_style=False, sort_keys=False),
            encoding="utf-8",
        )
        logger.warning("Conflict signal written: %s (%d files)", conflict_id, len(files))
